...
#3.) RETURNS "True", IF ANY ITEM, IN THE LIST, IS EMPTY, OR "False", IF NO LIST ITEMS, ARE EMPTY
def any_empty(LIST):
    #TODO: ADD A TYPE CHECK, FOR LIST
    for ITEM in LIST:
...
#3.) RETURNS THE LIST, WITH ANY STRING TYPE ITEM/S, STRIPPED
def strip_all(LIST):
    if any_empty([LIST]):
        raise ValueError('[ValueError]\nFunction: "strip_all()"\nThe supplied list parameter value, was empty.')
    elif not isinstance(LIST, list):
        raise TypeError('[ValueError]\nFunction: "strip_all()"\nThe supplied list parameter value, was not a list type.')
    else:
...
def db_file_path_error_check(DB_FILE_PATH):
    if any_empty([DB_FILE_PATH]):
        ...
        raise TypeError('[ValueError]\nFunction: "strip_all()"\nThe supplied database file path parameter value, was not a string type.')
...
def unquote(STRING):
    if not any_empty([STRING]):
        raise ValueError('[ValueError]\nFunction: "unquote()"\nThe supplied string parameter value, was empty.')
    if not isinstance(STRING, str):
        raise TypeError('[TypeError]\nFunction: "unquote()"\nThe supplied string parameter value, was not a string type.')
    if STRING[0] == STRING[-1] and STRING[0] in ("'", '"') and STRING.count(STRING[0]) == 2:
...
#THIS FUNCTION:
#1.) REQUIRES A STRING AND 32 BIT PASSWORD HASH BYTES
...
def aes_128_encrypt(STRING, PASSWORD_HASH_BYTES):
    if any_empty([STRING, PASSWORD_HASH_BYTES]):
        ...
        raise TypeError(f'[TypeError]\nFunction: "aes_128_encrypt()"\nThe supplied password hash bytes parameter value, was not a bytes type.')
    CIPHER = Fernet(urlsafe_b64encode(PASSWORD_HASH_BYTES))
    ENCRYPTED_STRING_BYTES = CIPHER.encrypt(STRING.encode())
...
#5.) RETURNS THE STRING HASH BYTES AND SALT BYTES, AS A TUPLE, OR RAISES AN ERROR
...
def get_hash_and_salt(STRING, SALT_BYTES=None):
    if any_empty([STRING]):
        ...
        raise TypeError('[TypeError]\nFunction: "get_hash_and_salt()"\nThe supplied salt bytes parameter value, was not a bytes type.')
    SALT_BYTES = token_bytes(16) if SALT_BYTES is None else SALT_BYTES
    HASH_BYTES = PBKDF2HMAC(
        algorithm=hashes.SHA256(),
        length=32, #(THE NUMBER OF BITS, IN LENGTH, THE HASH SHOULD BE)
        salt=SALT_BYTES,
        iterations=200_000 #(THE NUMBER OF TIMES THE STRING AND SALT BYTES, ARE HASHED)
    )
    STRING_BYTES = STRING.encode()

    #NOTE: THE STRING, MUST BE ENCODED, BEFORE DERIVING A HASH FOR IT
...
def table_exists(DB_FILE_PATH, DB_TABLE_NAME):
        try:
            ...
            #TUPLES, ARE RECOMMENDED, FOR USE WITH LISTS, OF 1-3 ITEMS.
...
#THIS FUNCTION:
#1.) REQUIRES DATABASE FILE PATH STRING, DB TABLE NAME STRING, COLUMN/S STRING, AND ROW VALUE/S TUPLE/LIST
...
def insert_row_values(DB_FILE_PATH, DB_TABLE_NAME, COLUMNS, ROW_VALUES):
    try:
        if any_empty([DB_FILE_PATH, DB_TABLE_NAME, COLUMNS, ROW_VALUES]):
            ...
            raise TypeError('[TypeError]\nFunction: "insert_row_values()"\nOnly the row values parameter value, should be a non-string type.')
        elif not isinstance(ROW_VALUES, tuple) and not isinstance(ROW_VALUES, list):
            raise TypeError('[TypeError]\nFunction: "insert_row_values()"\nThe supplied row values parameter value, was not a tuple or list type.')
        db_file_path_error_check(DB_FILE_PATH)
        ...
        #MUST BE A TUPLE OR LIST, BEFORE QUERYING THE "sqlite3" DB,
        #WITH A PARAMETERIZED QUERY, CONTAINING THOSE ROW VALUES.
        #WHEN PARAMETERIZING A SINGLE VALUE, IN A TUPLE, PLACE A COMMA,
        #AFTER THE VALUE, SO THE TUPLE, IS AN ACTUAL TUPLE TYPE (GENERAL RULE, FOR PARAMETERIZED QUERIES).
...
#THIS FUNCTION:
#1.) REQUIRES DATABASE FILE PATH STRING, DB TABLE NAME STRING, COLUMN/S LIST, ROW ID COLUMN STRING, AND ROW ID VALUE STRING
#2.) SELECTS 1 OR MORE ROW VALUES AND ALLOWS SPECIFYING THE WILDCARD VALUE ("*"),
#IN THE "COLUMNS" TUPLE/LIST PARAMETER, TO SELECT ALL COLUMNS
