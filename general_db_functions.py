...
#THIS FUNCTION:
#1.) REQUIRES A PASSWORD STRING, DATABASE STORED BASE64 ENCODE HASH STRING, AND DB STORED BASE64 ENCODED SALT STRING
...
def is_password_verified(PASSWORD, DB_STORED_BASE64_ENCODED_HASH, DB_STORED_BASE64_ENCODED_SALT):
    try:
...
def insert_row_values(DB_FILE_PATH, DB_TABLE_NAME, COLUMNS, ROW_VALUES):
    try:
        ...
        ROW_VALUES_LIST = list(str(VALUE).strip() for VALUE in ROW_VALUES.split(','))
