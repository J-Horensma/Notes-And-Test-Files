...
def aes_128_encrypt(STRING, URL_SAFE_BASE64_ENCODED_PASSWORD_HASH):
    if not all((isinstance(VALUE, str) for VALUE in (STRING, URL_SAFE_BASE64_ENCODED_PASSWORD_HASH))):
        raise TypeError(f'[TypeError]\nFunction: "aes_128_encrypt()"\nOne or more, supplied parameter/s, were not the correct type.')
    if not all((VALUE.strip() for VALUE in (STRING, URL_SAFE_BASE64_ENCODED_PASSWORD_HASH))):
        raise ValueError(f'[ValueError]\nFunction: "aes_128_encrypt()"\nOne or more, supplied parameter/s, were empty.')
    CIPHER = Fernet(URL_SAFE_BASE64_ENCODED_PASSWORD_HASH)
    STRING_BYTES = STRING.encode()
    ENCRYPTED_DECODED_STRING = CIPHER.encrypt(STRING_BYTES).decode('utf-8')
    return ENCRYPTED_DECODED_STRING
...
def aes_128_decrypt(ENCRYPTED_DECODED_STRING, URL_SAFE_BASE64_ENCODED_PASSWORD_HASH):
    if not all((isinstance(VALUE, str) for VALUE in (ENCRYPTED_DECODED_STRING, URL_SAFE_BASE64_ENCODED_PASSWORD_HASH))):
        raise TypeError(f'[TypeError]\nFunction: "aes_128_decrypt()"\nOne or more, supplied parameter/s, were not the correct type.')
    if not all((VALUE.strip() for VALUE in (ENCRYPTED_DECODED_STRING, URL_SAFE_BASE64_ENCODED_PASSWORD_HASH))):
        raise ValueError(f'[ValueError]\nFunction: "aes_128_decrypt()"\nThe supplied encrypted string parameter, was empty.')
    CIPHER = Fernet(URL_SAFE_BASE64_ENCODED_PASSWORD_HASH)
    PLAINTEXT_STRING = CIPHER.decrypt(ENCRYPTED_DECODED_STRING.encode()).decode('utf-8')
...
def get_hash_and_salt(STRING, SALT_BYTES=None):
    ...
    URL_SAFE_BASE64_ENCODED_HASH_STRING = urlsafe_b64encode(HASH_BYTES.derive(STRING_BYTES)).decode('utf-8')
    URL_SAFE_BASE64_ENCODED_SALT_STRING = urlsafe_b64encode(SALT_BYTES).decode('utf-8')
    return (URL_SAFE_BASE64_ENCODED_HASH_STRING, URL_SAFE_BASE64_ENCODED_SALT_STRING)
