...
#3.) RETURNS THE AES-128 ENCRYPTED BYTES
def aes_128_encrypt(STRING, PASSWORD_HASH_BYTES):
    ...
    if not all((VALUE.strip() for VALUE in (STRING, PASSWORD_HASH_BYTES.decode('utf-8')))):
        raise ValueError(f'[ValueError]\nFunction: "aes_128_encrypt()"\nOne or more, supplied parameter/s, were empty.')
    CIPHER = Fernet(PASSWORD_HASH_BYTES)
...
#2.) DECRYPTS THE AES-128 ENCRYPTED BYTES, USING THE SUPPLIED PASSWORD HASH BYTES,
#THAT WAS USED TO ENCRYPT THE DATA, AND DECODES THE DECRYPTED BYTES TO A STRING
#3.) RETURNS THE DECRYPTED STRING
def aes_128_decrypt(ENCRYPTED_BYTES, PASSWORD_HASH_BYTES):
    if not all((isinstance(ENCRYPTED_BYTES, bytes), isinstance(PASSWORD_HASH_BYTES, bytes))):
        raise TypeError(f'[TypeError]\nFunction: "aes_128_decrypt()"\nOne or more, supplied parameter/s, were not a bytes type.')
    if not all((VALUE.strip() for VALUE in (ENCRYPTED_BYTES.decode('utf-8'), PASSWORD_HASH_BYTES.decode('utf-8')))):
        raise ValueError(f'[ValueError]\nFunction: "aes_128_decrypt()"\nOne or more, supplied parameter/s, were empty.')
    CIPHER = Fernet(PASSWORD_HASH_BYTES)
    DECRYPTED_STRING = CIPHER.decrypt(ENCRYPTED_BYTES).decode('utf-8')
...
#5.) RETURNS THE STRING HASH BYTES AND SALT HASH BYTES, AS A LIST
...
def get_hash_and_salt(STRING, SALT=None):
    SALT_BYTES = token_bytes(16) if SALT is None else SALT
    ...
    HASH_BYTES = pbkdf2_hmac(
        ...
        STRING_BYTES,
        SALT_BYTES,
        200_000 #(THE NUMBER OF TIMES THE STRING AND SALT BYTES, ARE HASHED)
    )
    HASH_AND_SALT = (HASH_BYTES, SALT_BYTES)
...
def is_password_valid(PASSWORD, DB_STORED_HASH_BYTES, DB_STORED_SALT_BYTES):
    try:
        NEW_HASH_BYTES, _ = get_hash_and_salt(PASSWORD, DB_STORED_SALT_BYTES)
...
def insert_row_values(DB_FILE_PATH, DB_TABLE_NAME, COLUMNS, ROW_VALUES):
    try:
        if not all(isinstance(VALUE, str) for VALUE in [DB_FILE_PATH, DB_TABLE_NAME, COLUMNS, ROW_VALUES]):
        ...
        #BY THE "sqlite3" DB.
        ROW_VALUES_LIST = [VALUE.strip() for VALUE in ROW_VALUES.split(',')]
...
def select_row_values(DB_FILE_PATH, DB_TABLE_NAME, COLUMNS, ROW_ID_COLUMN, ROW_ID_VALUE):
    try:
        if not all(isinstance(VALUE, str) for VALUE in [DB_FILE_PATH, DB_TABLE_NAME, COLUMNS, ROW_ID_COLUMN, ROW_ID_VALUE]):
            raise TypeError('[TypeError]\nFunction: "select_row_values()"\nOne or more, supplied parameter/s, were not a string type.')
        if not all(VALUE.strip() for VALUE in [DB_FILE_PATH, DB_TABLE_NAME, COLUMNS, ROW_ID_COLUMN, ROW_ID_VALUE]):
            raise ValueError('[ValueError]\nFunction: "select_row_values()"\nOne or more, supplied parameter/s, were empty.')
        db_file_path_error_check(DB_FILE_PATH)
...
def update_row_values(DB_FILE_PATH, DB_TABLE_NAME, COLUMNS, ROW_VALUES, ROW_ID_COLUMN, ROW_ID_VALUE):
    try:
        if not all(isinstance(VALUE, str) for VALUE in [DB_FILE_PATH, DB_TABLE_NAME, COLUMNS, ROW_VALUES, ROW_ID_COLUMN, ROW_ID_VALUE]):
            raise TypeError('[TypeError]\nFunction: "update_row_values()"\nOne or more, supplied parameter/s, were not a string type.')
        if not all(VALUE.strip() for VALUE in [DB_FILE_PATH, DB_TABLE_NAME, COLUMNS, ROW_VALUES, ROW_ID_COLUMN, ROW_ID_VALUE]):
            raise ValueError('[ValueError]\nFunction: "update_row_values()"\nOne or more, supplied parameter/s, were empty.')
        db_file_path_error_check(DB_FILE_PATH)
        if not bool(fullmatch(r'[A-Za-z0-9_]+', DB_TABLE_NAME)) and not is_quoted(DB_TABLE_NAME):
...
def delete_row(DB_FILE_PATH, DB_TABLE_NAME, ROW_ID_COLUMN, ROW_ID_VALUE):
    try:
        if not all(isinstance(VALUE, str) for VALUE in [DB_FILE_PATH, DB_TABLE_NAME, ROW_ID_COLUMN, ROW_ID_VALUE]):
            raise TypeError('[TypeError]\nFunction: "delete_row()"\nOne or more, supplied parameter/s, were not a string type.')
        if not all(VALUE.strip() for VALUE in [DB_FILE_PATH, DB_TABLE_NAME, ROW_ID_COLUMN, ROW_ID_VALUE]):
            raise ValueError('[ValueError]\nFunction: "delete_row()"\nOne or more, supplied parameter/s, were empty.')
        db_file_path_error_check(DB_FILE_PATH)
        if not bool(fullmatch(r'[A-Za-z0-9_]+', DB_TABLE_NAME)) and not is_quoted(DB_TABLE_NAME):
