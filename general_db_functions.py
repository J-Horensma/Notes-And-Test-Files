...
#3.) RETURNS "True", IF ANY ITEM, IN THE LIST, IS EMPTY, "False", IF NO LIST ITEMS, ARE EMPTY
def any_empty(LIST):
    for ITEM in LIST:
...
#3.) RETURNS A LIST, WITH ANY STRING TYPE ITEM/S, STRIPPED
def strip_all(LIST):
...
#3.) RETURNS THE DECRYPTED PLAINTEXT STRING OR ANY ERRORS FOUND
def aes_128_decrypt(ENCRYPTED_STRING_BYTES, HASH_BYTES):
    if any_empty([ENCRYPTED_STRING_BYTES, PASSWORD_HASH_BYTES]):
        raise ValueError('[ValueError]\nFunction: "aes_128_decrypt()"\nOne or more supplied parameters, were empty.')
    if not all((isinstance(VALUE, bytes) for VALUE in (ENCRYPTED_STRING_BYTES, PASSWORD_HASH_BYTES))):
        raise TypeError(f'[TypeError]\nFunction: "aes_128_decrypt()"\nOne or more, supplied parameters, were not a bytes type.')
