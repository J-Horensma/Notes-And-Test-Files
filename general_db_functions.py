...
def encrypt_data(DATA, PASSWORD_HASH):
    if not all((isinstance(DATA, str), isinstance(PASSWORD_HASH, bytes))):
        raise TypeError(f'[TypeError]\nFunction: "encrypt_data()"\nOne or more, supplied parameter/s, were not the correct type.')
    if not all((VALUE.strip() for VALUE in (DATA, PASSWORD_HASH.decode('utf-8')))):
...
def decrypt_data(ENCRYPTED_DATA, PASSWORD_HASH):
    if not all((isinstance(ENCRYPTED_DATA, bytes), isinstance(PASSWORD_HASH, bytes))):
        raise TypeError(f'[TypeError]\nFunction: "decrypt_data()"\nOne or more, supplied parameter/s, were not a bytes type.')
    if not all((VALUE.strip() for VALUE in (ENCRYPTED_DATA.decode('utf-8'), PASSWORD_HASH.decode('utf-8')))):
        raise ValueError(f'[ValueError]\nFunction: "decrypt_data()"\nOne or more, supplied parameter/s, were empty.')
    CIPHER = Fernet(PASSWORD_HASH)
...
#2.) INSERTS 1 OR MORE ROW VALUES, INTO THE COLUMN NAME/S
#3.) RAISES ANY ERRORS
...
#THIS FUNCTION:
#1.) REQUIRES DATABASE FILE PATH, DB TABLE NAME, COLUMN/S, ROW VALUES, ROW ID COLUMN, AND ROW ID VALUE STRINGS
#2.) UPDATES 1 OR MORE ROW VALUES, IN THE COLUMN/S NAME/S, OF THE ROW IDENTIFIED BY THE ROW ID COLUMN AND ROW ID VALUE
#3.) RAISES ANY ERRORS
