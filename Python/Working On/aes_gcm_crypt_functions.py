...
#4.) A VARIABLE SIZE PASSWORD HASH, IS GENERATED, AS A KEY, FOR AES-GCM CRYPTOGRAPHY
...
#2.) CHECKS FOR AES-GCM HEADERS, IN THE FILE, AND RETURNS THE HEADERS, AS A LIST, OR AN ERROR
def check_aes_gcm_headers(FILE, KEY_SIZE):
    if not isinstance(FILE, IOBase) or 'r' not in FILE.mode or 'b' not in FILE.mode:
        ...
        raise TypeError('[TypeError]\nFunction: "check_aes_gcm_headers()"\nThe key size parameter, must be an integer.')
    elif KEY_SIZE not in [128, 192, 256]:
        raise ValueError('[ValueError]\nFunction: "check_aes_gcm_headers()"\nThe key size parameter, must be 128, 192, or 256.')
    else:
        try:
            ...
            if ALGORITHM_AND_MODE_HEADER_SIZE != len(ALGORITHM_AND_MODE_HEADER):
                FILE.seek(0); return [None, 'ALGORITHM_AND_MODE_HEADER_CORRUPTED']
            #CHECK IF THE FILE, IS AES-GCM ENCRYPTED
            elif ALGORITHM_AND_MODE_HEADER.decode() != 'AES-GCM':
            ...
            if KEY_SIZE_HEADER_SIZE != len(KEY_SIZE_HEADER):
                FILE.seek(0); return [None, 'KEY_SIZE_HEADER_CORRUPTED']
            #CHECK IF THE FILE, IS ENCRYPTED WITH THE SAME KEY SIZE
            elif KEY_SIZE != int(KEY_SIZE_HEADER.decode()):
                FILE.seek(0); return [None, 'DIFFERENT_AES-GCM_KEY_SIZE']
            ...
            FILE.seek(0); return [NONCE_BYTES_HEADER, TAG_BYTES_HEADER, SALT_BYTES_HEADER, TOTAL_HEADERS_SIZE]
        except Exception as ERROR:
            raise Exception(f'[Exception]\nFunction: "check_aes_gcm_headers()"\n{ERROR}')
        
#THIS FUNCTION:
#1.) REQUIRES A FOLDER PATH STRING, KEY SIZE INTEGER, AND PASSWORD STRING
#2.) RECURSIVELY AES-GCM ENCRYPTS ALL FILES, WITHIN THE SUPPLIED FOLDER PATH (ANY FILE TYPE)
...
def aes_gcm_decrypt_folder(FOLDER_PATH, KEY_SIZE, PASSWORD):
    KEY_SIZE_LIST = [128, 192, 256]
    if not isinstance(FOLDER_PATH, str):
    ...
    try:
        for ROOT, DIRECTORIES, FILES in walk(FOLDER_PATH):
            ...
            for FILE_NAME in FILES:
                FILE_PATH = join(ROOT, FILE_NAME)
                RESULT = aes_gcm_decrypt_file(FILE_PATH, KEY_SIZE, PASSWORD)
                if not RESULT[0]:
                    ERRORS.append(RESULT[1])
        if ERRORS:
            return [False, ERRORS]
        else:
            return [True, f'AES-GCM_FOLDER_DECRYPTION_SUCCESS:\n{FOLDER_PATH}']
    except Exception as ERROR:
        raise Exception(f'[Exception]\nFunction: "aes_gcm_decrypt_folder()"\n{ERROR}\n{FOLDER_PATH}')

#1.) REQUIRES A FILE PATH STRING, KEY SIZE INTEGER, AND PASSWORD STRING
#2.) ACCEPTS AN OPTIONAL BLOCK SIZE INTEGER
#3.) AES-GCM ENCRYPTS THE SUPPLIED FILE PATH, WITH THE SUPPLIED KEY SIZE (ANY FILE TYPE)
def aes_gcm_encrypt_file(FILE_PATH, KEY_SIZE, PASSWORD, BLOCK_SIZE=None):
    ...
    try:
        ...
        with open(FILE_PATH, 'rb') as INFILE:
            ...
            AES_GCM_HEADERS_CHECK = check_aes_gcm_headers(INFILE, KEY_SIZE)
        ...
        if exists(FILE_PATH + '.tmp') and all([access(FILE_PATH + '.tmp', W_OK), access(FILE_PATH + '.tmp', X_OK)]):
...
def aes_gcm_decrypt_file(FILE_PATH, KEY_SIZE, PASSWORD, BLOCK_SIZE=None):
    KEY_SIZE_LIST = [128, 192, 256]
    if not isinstance(FILE_PATH, str):
    ...
    try:
        ...
        with open(FILE_PATH, 'rb') as INFILE:
            ...
            NONCE_BYTES, TAG_BYTES, SALT_BYTES, TOTAL_HEADERS_SIZE = AES_GCM_HEADERS_CHECK
            ...
            with open(FILE_PATH + '.tmp', 'wb') as OUTFILE:
                ...
                while True:
                    ...
                    if CHUNK_SIZE <= 0:
                        if exists(FILE_PATH + '.tmp') and all([access(FILE_PATH + '.tmp', W_OK), access(FILE_PATH + '.tmp', X_OK)]):
                            remove(FILE_PATH + '.tmp')
                            return [False, f'AES_GCM_ENCRYPTED_DATA_SIZE_DATA_CORRUPTED:\n{FILE_PATH}']
                        else:
                            return [False, f'AES_GCM_ENCRYPTED_DATA_SIZE_DATA_CORRUPTED:\n{FILE_PATH}\nAdditionally, the temporary file: {FILE_PATH + ".tmp"},\nwas unable to be deleted.']
                    ...
                    if len(ENCRYPTED_CHUNK) != CHUNK_SIZE:
                        if exists(FILE_PATH + '.tmp') and all([access(FILE_PATH + '.tmp', W_OK), access(FILE_PATH + '.tmp', X_OK)]):
                            remove(FILE_PATH + '.tmp')
                            return [False, f'AES_GCM_ENCRYPTED_DATA_CORRUPTED:\n{FILE_PATH}']
                        else:
                            return [False, f'AES_GCM_ENCRYPTED_DATA_CORRUPTED:\n{FILE_PATH}\nAdditionally, the temporary file: {FILE_PATH + ".tmp"},\nwas unable to be deleted.']
                    PLAINTEXT_CHUNK = DECRYPTOR.update(ENCRYPTED_CHUNK) + DECRYPTOR.finalize()
...
aes_gcm_encrypt_file(r"C:\Users\jdhor\Desktop\Cryptography\AES-128\Modules\Cryptography\Tests\File Cryptor (GUI)\Test Files\test.txt", 256, 'Test')
input('Press Enter: ')
aes_gcm_decrypt_file(r"C:\Users\jdhor\Desktop\Cryptography\AES-128\Modules\Cryptography\Tests\File Cryptor (GUI)\Test Files\test.txt", 256, 'Test')