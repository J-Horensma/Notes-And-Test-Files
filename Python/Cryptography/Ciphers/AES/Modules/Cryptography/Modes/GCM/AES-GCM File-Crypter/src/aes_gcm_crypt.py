...
__description__ = 'An AES-GCM cryptography library, for cryptography of variables, files, and folders (Cross-platform).'
...
def aes_gcm_encrypt_file(FILE_PATH, KEY_SIZE, PASSWORD, BLOCK_SIZE=None):
    ...
    try:
        ...
        #CHECK IF READ, WRITE, AND EXECUTE FILE PERMISSION, ARE AVAILABLE
        if not access(FILE_PATH, R_OK | W_OK | X_OK):
            try:
                #IF READ, WRITE, AND EXECUTE FILE PERMISSION, ARE NOT AVAILABLE,
                #ATTEMPT TO SET THEM
        ...
        try:
            ...
            raise Exception(f'[Exception]\nFunction: "aes_gcm_encrypt_file()"\n{ERROR}\nAdditionally, the temporary file: "{FILE_PATH + '.tmp'}", was unable to be deleted.')
        raise Exception(f'[Exception]\nFunction: "aes_gcm_encrypt_file()"\n{ERROR}\nFile path: {FILE_PATH}')
