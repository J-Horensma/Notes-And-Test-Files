...
from tkinter import Tk, ttk, Frame, PhotoImage, scrolledtext, Button, messagebox
...
'''
                 ...
                 Copyright (C) 2026 Joel Horensma
...
ICO = join(BASE_DIRECTORY, normpath('assets/icon/ico/icon.ico'))
PNG = join(BASE_DIRECTORY, normpath('assets/icon/png/256x256.png'))
if sys.platform.startswith('win'):
    try:
        ROOT_WINDOW.iconbitmap(ICO)
    except Exception:
        IMAGE = PhotoImage(file=PNG)
        ROOT_WINDOW.iconphoto(True, IMAGE)
elif sys.platform.startswith('darwin'):
        IMAGE = PhotoImage(file=ICO)
        ROOT_WINDOW.iconphoto(True, IMAGE)
else:
    IMAGE = PhotoImage(file=PNG)
    ROOT_WINDOW.iconphoto(True, IMAGE)
