...
#THIS FUNCTION:
#1.) REQUIRES A "tkinter.Tk()" OR "tkinter.Toplevel()" WINDOW CLASS, A WINDOW WIDTH INTEGER, AND A WINDOW HEIGHT INTEGER
...
def center_window(WINDOW, WINDOW_WIDTH, WINDOW_HEIGHT):
    if not isinstance(WINDOW, (Tk, Toplevel)):
        raise TypeError('[TypeError]\nFunction: "center_window()"\nThe window parameter, must be a "tkinter.Tk()" or "tkinter.Toplevel()" class.')
