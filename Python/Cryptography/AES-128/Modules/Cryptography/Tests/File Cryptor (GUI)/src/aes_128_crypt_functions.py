...
#3.) RETURNS A LIST, CONTAINING THE ENCRYPTED VARIABLE BYTES AND A 16 BYTE SALT BYTES
...
#THIS FUNCTION:
#1.) REQUIRES AES-128 ENCRYPTED BYTES, A PASSWORD STRING, AND THE SALT BYTES
...
def aes_128_decrypt_variable(ENCRYPTED_VARIABLE_BYTES, PASSWORD, SALT_BYTES):
    if not isinstance(ENCRYPTED_VARIABLE_BYTES, bytes):
        ...
        raise ValueError('[ValueError]\nFunction: "aes_128_decrypt_variable()"\nThe supplied salt bytes parameter, was not 16 bytes long.')
    try:
        HASH_BYTES, SALT_BYTES = get_hash_and_salt(PASSWORD, SALT_BYTES)
        CIPHER = Fernet(urlsafe_b64encode(HASH_BYTES))
