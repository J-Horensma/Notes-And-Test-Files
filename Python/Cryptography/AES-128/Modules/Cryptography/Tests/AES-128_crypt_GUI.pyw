...
def aes_128_encrypt_folder_thread(WINDOW, ENCRYPT_FOLDER_BUTTON, DECRYPT_FOLDER_BUTTON):
    ...
    def finish_process(RESULT):
        ...
        if RESULT[0]:
            PROGRESS_BAR_MESSAGE.config(text=f'Successfully AES-128 encrypted, the folder:\n{FOLDER_PATH}')
            WINDOW.after(5000, PROGRESS_BAR_MESSAGE.pack_forget)
        elif isinstance(RESULT[1], list):
            ERROR_TEXT = '\n'.join(RESULT[1])  
            PROGRESS_BAR_MESSAGE.config(text=ERROR_TEXT)
            WINDOW.after(5000, PROGRESS_BAR_MESSAGE.pack_forget)
...
def aes_128_decrypt_folder_thread(WINDOW, ENCRYPT_FOLDER_BUTTON, DECRYPT_FOLDER_BUTTON):    
    ...
    def finish_process(RESULT):
        ...
        PROGRESS_BAR.pack_forget()
        if RESULT[0]:
            PROGRESS_BAR_MESSAGE.config(text=f'Successfully AES-128 decrypted, the folder:\n{FOLDER_PATH}')
