...
def get_screen_size(WINDOW):
    if not isinstance(WINDOW, Tk):
        raise TypeError('[TypeError]\nFunction: "get_screen_size()"\nThe supplied root window parameter, was not a valid "tkinter.Tk()" class.')
...
#THIS FUNCTION:
#1.) REQUIRES A "tkinter.Tk()" CLASS, WINDOW WIDTH, AND HEIGHT INTEGERS
...
#THIS FUNCTION:
#1.) ACCEPTS A PROMPT TITLE STRING
#2.) ACCEPTS A PROMPT PATH STRING
#3.) PROMPTS THE USER TO SELECT A FOLDER PATH
#4.) RETURNS THE FULL FOLDER PATH, THAT WAS CHOSEN, AS A STRING
...
#1.) ACCEPTS A PROMPT TITLE STRING
#2.) ACCEPTS A PROMPT PATH STRING
#3.) ACCEPTS A FILE TYPES LIST, USING TUPLES
#FORMAT: [('Text Files', '*.txt'), ('Python Files', '*.py')]
#4.) PROMPTS THE USER TO CHOOSE A FILE PATH
#5.) RETURNS THE FULL FILE PATH, THAT WAS CHOSEN, AS A STRING
...
#THIS FUNCTION:
#1.) ACCEPTS A TKINTER WINDOW
#2.) ACCEPTS A PROMPT TITLE STRING
#3.) PROMPTS THE USER TO ENTER A STRING
#4.) RETURNS THE STRING, THAT WAS ENTERED
def input_string_prompt(WINDOW, PROMPT_TITLE=None):
    ...
    while True:
        ...
        if not STRING:
            messagebox.showerror('Error!', 'An input, must be supplied.')
            continue
        else:
            break
    return STRING