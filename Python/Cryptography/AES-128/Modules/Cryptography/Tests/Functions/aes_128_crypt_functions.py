...
from os import walk, replace, urandom, fsync
...
def get_hash_and_salt(PASSWORD, SALT_BYTES=None):
    ...
    ALGORITHM = PBKDF2HMAC(
        algorithm=SHA256(),
        length=32, #THE NUMBER OF BYTES, THE RETURNED HASH, WILL BE
        salt=SALT_BYTES,
...
def aes_128_encrypt_folder(FOLDER_PATH, PASSWORD):
    ...
    for ROOT, DIRECTORIES, FILES in walk(FOLDER_PATH):
        ...
        for FILE_NAME in FILES:
            ...
            ENCRYPTION_ATTEMPT = aes_128_encrypt_file(FILE_PATH, PASSWORD)
            if ENCRYPTION_ATTEMPT[0] is False:
                return [False, ENCRYPTION_ATTEMPT[1]]
...
def aes_128_decrypt_folder(FOLDER_PATH, PASSWORD):
    ...
    for ROOT, DIRECTORIES, FILES in walk(FOLDER_PATH):
        ...
        for FILE_NAME in FILES:
            FILE_PATH = join(ROOT, FILE_NAME)
            DECRYPTION_ATTEMPT = aes_128_decrypt_file(FILE_PATH, PASSWORD)
            if DECRYPTION_ATTEMPT[0] is False:
                return [False, DECRYPTION_ATTEMPT[1]]
...
def check_aes_128_encryption_headers(FILE):
    ...
    FILE.seek(0)
    while True:
        #CHECK IF THE FILE, IS AES-128 ENCRYPTED
        SIZE_DATA = FILE.read(4)
        if len(SIZE_DATA) < 4:
            FILE.seek(0)
...
def aes_128_encrypt_file(FILE_PATH, PASSWORD, BLOCK_SIZE=None):
    ...
    #DELETE THE PASSWORD VARIABLE, CONTAINING THE USER-ENTERED PASSWORD, FROM THE MEMORY
    del PASSWORD
    ...
    with open(FILE_PATH, 'rb') as INFILE, open(FILE_PATH + '.tmp', 'wb') as OUTFILE:
        ...
        START_AES_128_ENCRYPTION_HEADERS = b'START_AES_128_ENCRYPTION_HEADERS'
        END_AES_128_ENCRYPTION_HEADERS = b'END_AES_128_ENCRYPTION_HEADERS'
        OUTFILE.write(pack('>I', len(START_AES_128_ENCRYPTION_HEADERS)) + START_AES_128_ENCRYPTION_HEADERS)
        OUTFILE.write(pack('>I', len(SALT_BYTES)) + SALT_BYTES)
        OUTFILE.write(pack('>I', len(END_AES_128_ENCRYPTION_HEADERS)) + END_AES_128_ENCRYPTION_HEADERS)
    ...
    try:
        ...
        return [False, 'PLAINTEXT_FILE_WIPE_FAILED']
...
def aes_128_decrypt_file(FILE_PATH, PASSWORD, BLOCK_SIZE=None):
    ...
    with open(FILE_PATH, 'rb') as INFILE, open(FILE_PATH + '.tmp', 'wb') as OUTFILE:
        ...
        if CHECK_HEADERS_RESULT[1] == 'FILE_EMPTY':
            return [False, CHECK_HEADERS_RESULT[1]]
        elif CHECK_HEADERS_RESULT[0] is None:
            return [False, CHECK_HEADERS_RESULT[1]]
        else:
        ...
        while True:
            ...
            try:
                ...
                return [False, 'AES_128_FILE_DECRYPTION_FAILED']
