    ...
    for ROOT, DIRECTORIES, FILES in walk(FOLDER_PATH):
        ...
        for FILE_NAME in FILES:
            ...
            if 'FILE_EMPTY' in RESULT[1]:
                ...
                ERRORS.append(RESULT[1])
            elif 'AES_128_FILE_ENCRYPTION_FAILURE' in RESULT[1]:
                ERRORS.append(RESULT[1])
    if ERRORS:
        return [False, ERRORS]
    return [True, f'AES_128_FOLDER_ENCRYPTION_SUCCESS\nFolder Encryption Success Path:\n{FOLDER_PATH}']
...
def aes_128_encrypt_file(FILE_PATH, PASSWORD, BLOCK_SIZE=None):
    try:
        ...
        #TO PREVENT LOADING ANY OF THE PLAINTEXT FILE DATA, TO THE MEMORY, AS VARIABLES
        ...
        return [True, f'AES_128_FILE_ENCRYPTION_SUCCESS:\n{FILE_PATH}']
    except Exception as ERROR:
        #ENSURE THE ".tmp" FILE IS REMOVED, ON AES-128 ENCRYPTION FAILURE
        try:
            remove(FILE_PATH + '.tmp')
            return [False, f'AES_128_FILE_ENCRYPTION_FAILURE:\n{FILE_PATH}']
        except:
            return [False, f'AES_128_FILE_ENCRYPTION_FAILURE:\n{FILE_PATH}\nAdditionally, the ".tmp" file could not be removed, and may need to be manually deleted.']
