...
#THIS FUNCTION:
#1.) REQUIRES A LIST
#2.) CHECKS IF ANY ITEM, IN THE LIST, IS EMPTY, THEN CONTINUES TO THE NEXT ITEM (EVEN A BYTES TYPE)
#3.) RETURNS "True", IF ANY ITEM, IN THE LIST, IS EMPTY, "False", IF NO LIST ITEMS, ARE EMPTY, OR RAISES A "TypeError",
#IF THE LIST PARAMETER, IS NOT A LIST TYPE
def any_empty(LIST):
...
#THIS FUNCTION:
#1.) REQUIRES A LIST
#2.) CHECKS EVERY ITEM, IN THE LIST, AND STRIPS ANY STRING TYPE ITEM/S (SKIPS ANY NON-STRING TYPE ITEM/S)
...
def db_file_path_error_check(DB_FILE_PATH):
    if any_empty([DB_FILE_PATH]):
        ...
        raise FileNotFoundError(f'[FileNotFoundError]\nFunction: "{currentframe().f_back.f_code.co_name}()"\nThe supplied database file path parameter, was not an existing path.')
...
#3.) RETURNS THE STRING, UNQUOTED, OR RAISES AN ERROR, IF THE STRING PARAMETER, IS NOT A STRING TYPE
def unquote(STRING):
    if any_empty([STRING]):
...
#THIS FUNCTION:
#1.) REQUIRES A STRING OR BYTES TYPE STRING PARAMETER AND A 32 BIT PASSWORD HASH BYTES
...
#3.) RETURNS THE STRING, AES-128 ENCRYPTED, OR RAISES AN ERROR
...
#THIS FUNCTION:
#1.) REQUIRES AES-128 ENCRYPTED BYTES AND PASSWORD HASH BYTES
#2.) DECRYPTS THE AES-128 ENCRYPTED BYTES, USING THE PASSWORD HASH BYTES VALUE,
...
#THIS FUNCTION:
#1.) REQUIRES A STRING OR BYTES TYPE STRING PARAMETER AND ACCEPTS AN OPTIONAL SALT BYTES PARAMETER
...
#4.) HASHES THE BYTES, WITH THE PBKDF2 HMAC HASHING ALGORITHM, 
#USING THE "SHA256" HASH FUNCTION, WITH A SALT, AND 200,000 ITERATIONS
#5.) RETURNS THE STRING PARAMETER HASH BYTES AND SALT BYTES, AS A LIST, OR RAISES AN ERROR
...
#THIS FUNCTION:
#1.) REQUIRES A PASSWORD STRING, DATABASE STORED PASSWORD HASH BYTES, AND DATABASE STORED SALT BYTES
#2.) CONVERTS THE PASSWORD STRING, TO HASH BYTES, USING THE DATABASE STORED SALT BYTES,
#SO THE RESULTING PASSWORD HASH, MATCHES THE DATABASE STORED HASH, IF THE SUPPLIED PASSWORD STRING, 
#IS THE SAME AS THE ORIGINAL PASSWORD STRING, USED TO CREATE THE DB STORED HASH BYTES
#3.) COMPARES THE PASSWORD HASH BYTES, TO THE DATABASE STORED PASSWORD HASH BYTES
#4.) RETURNS "True", IF THE PASSWORD HASHES MATCH, OR "False", IF THE PASSWORD HASHES, DO NOT MATCH OR IF AN ERROR OCCURS
...
#2.) CHECKS IF A DATABASE TABLE, ALREADY EXISTS
...
def table_exists(DB_FILE_PATH, DB_TABLE_NAME):
        try:
            ...
            BOOLEAN = DB_CONNECTION_CURSOR.execute('SELECT name FROM sqlite_master WHERE type="table" AND name=?;', (DB_TABLE_NAME,)).fetchone() is not None
...
#WITH THE COLUMN/S INFO PARAMETER, CONTAINING THE COLUMN NAME/S AND ATTRIBUTES, SEPARATED BY COMMAS,
#E.G.: 
#COLUMNS_INFO = '''
...
def create_table(DB_FILE_PATH, DB_TABLE_NAME, COLUMNS_INFO):
    try:
        ...
        if bool(fullmatch(r'[A-Za-z0-9_]+', DB_TABLE_NAME)) or is_quoted(DB_TABLE_NAME):
            UNQUOTED_DB_TABLE_NAME = unquote(DB_TABLE_NAME)
            if table_exists(DB_FILE_PATH, UNQUOTED_DB_TABLE_NAME):
                raise ValueError('[ValueError]\nFunction: "create_table()"\nThe supplied database table name, already exists, in the database.')
        else:
            raise ValueError('[ValueError]\nFunction: "create_table()"\nThe supplied database table name parameter, was invalid.\nWhen creating a database table, the table name, must be surrounded with quotes,\nif it includes characters other than letters, numbers, and underscores.')
        CONNECTION = connect(DB_FILE_PATH)
        CURSOR = CONNECTION.cursor()
        CURSOR.execute(f'CREATE TABLE {DB_TABLE_NAME} ({COLUMNS_INFO});')
        CONNECTION.commit()
        CONNECTION.close()
    except OperationalError as ERROR:
        raise OperationalError(f'[OperationalError]\nFunction: "create_table()"\n{ERROR}')

#THIS FUNCTION:
#1.) REQUIRES A DATABASE FILE PATH STRING, DATABASE TABLE NAME STRING, COLUMN/S STRING, AND ROW VALUE/S TUPLE/LIST
#2.) INSERTS 1 OR MORE ROW VALUE/S, INTO THE DATABASE TABLE
#3.) RAISES ANY ERRORS FOUND
...
#3.) RETURNS THE SELECTED ROW VALUE/S, AS A LIST, RETURNS "0", IF THE ROW, WAS NOT FOUND, OR RAISES AN ERROR
def select_row_values(DB_FILE_PATH, DB_TABLE_NAME, COLUMNS, ROW_ID_COLUMN, ROW_ID_VALUE):
    try:
        ...
        SELECTED_ROW_VALUES = CURSOR.execute(f'SELECT {COLUMNS} FROM {DB_TABLE_NAME} WHERE {ROW_ID_COLUMN} = ?;', ROW_ID_VALUE).fetchone()
        if SELECTED_ROW_VALUES is None:
            return 0
        SELECTED_ROW_VALUES = list(SELECTED_ROW_VALUES)
        CONNECTION.close()
...
#THIS FUNCTION:
#1.) REQUIRES DATABASE FILE PATH, DATABASE TABLE NAME, COLUMN/S, ROW VALUE/S, ROW ID COLUMN, AND ROW ID VALUE STRINGS
#2.) UPDATES 1 OR MORE ROW VALUE/S, IN THE ROW IDENTIFIED, BY THE ROW ID COLUMN AND ROW ID VALUE STRINGS
#3.) RAISES ANY ERRORS FOUND
...
#THIS FUNCTION:
#1.) REQUIRES DATABASE FILE PATH, DATABASE TABLE NAME, ROW ID COLUMN, AND ROW ID VALUE STRINGS
