...
while True:
    try:
        ...
        #THE "insert_row_values()" FUNCTION, REQUIRES THE ROW VALUES TO BE, IN A LIST.
        #THE ".split()" METHOD, CAN CONVERT A COMMA SEPARATED STRING OF ITEMS, INTO A LIST.
        ROW_VALUES = ROW_VALUES.split(',')
        
        insert_row_values(DB_FILE_PATH, DB_TABLE_NAME, COLUMNS, ROW_VALUES)
        print(f'The row value/s:\n{ROW_VALUES},')
        print(f'was/were inserted, into the column/s: {COLUMNS}')
        print(f'in the database table: "{unquote(DB_TABLE_NAME)}".\n')
